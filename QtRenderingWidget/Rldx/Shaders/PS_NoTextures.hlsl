...
float4 main(in PixelInputType input) : SV_TARGET
{
    ...
    float4 GlossTex = float4(0.8, 0.2, 0.0, 1); //shaderTextures[t_GlossMap].Sample(s_anisotropic, input.tex1);
...
//    }
    ...
    float4 color = float4(directLighting + ambientLighting/* * env_color.rgb * env_light_radiance*/, 1.0f);
