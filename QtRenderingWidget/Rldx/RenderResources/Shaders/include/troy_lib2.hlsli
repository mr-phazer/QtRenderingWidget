...
float linear_accurate_component(in const float srgb_val)
{
    ...
    else
    {
        ...
        return pow((abs(srgb_val + a)) / (1.0f + a), 2.4f);
    }
}
...
{
    return (tex_cube_specular.SampleLevel(s_anisotropic, ( /*texcoordEnvSwizzle*/(direction)), lod).rgb);
    //*
    //        env_color.rgb * env_light_radiance;
}
...
float3 standard_lighting_model_directional_light_SM4_private(in const float3 LightColor, in const float3 normalised_light_dir, in const float3 normalised_view_dir, in const float3 reflected_view_vec, in R2_4_StandardLightingModelMaterial material)
{
    ...
    dlight_diffuse *= get_diffuse_scale_factor();
...
{
    float direct_light_scale = 500.0f; //	The game is 500 units of HDR light for each 1 unit of LDR
