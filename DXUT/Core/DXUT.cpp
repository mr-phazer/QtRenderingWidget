...
protected:
    struct STATE
    {
        ...
        ID3D11RenderTargetView* m_D3D11RenderTargetView;   // the D3D11 renderQuad target view
        ...
        bool  m_IgnoreSizeChange;           // if true, DXUT won't resize the device upon HWND size change
        ...
        bool  m_DeviceLost;                 // if true, then the device is lost and needs to be resize
        ...
        LPDXUTCALLBACKD3D11SWAPCHAINRESIZED     m_D3D11SwapChainResizedFunc;    // D3D11 SwapChain resize callback
        ...
        LPDXUTCALLBACKD3D11FRAMERENDER          m_D3D11FrameRenderFunc;         // D3D11 frame renderQuad callback
        ...
        void* m_D3D11FrameRenderFuncUserContext;         // user context for D3D11 frame renderQuad callback
...
LRESULT CALLBACK DXUTStaticWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...

            {
                ...
                if (pd3dDevice)
                {
                    ...
                    if (DXGI_STATUS_OCCLUDED == hr)
                    {
                        ...
                        // Don't renderQuad until we're visible again.
        ...
        else
        {
            ...
            else if (SIZE_RESTORED == wParam)
            {
                ...
                else if (GetDXUTState().GetInSizeMove())
                {
                    ...
                    // case, we don't resize the device yet -- we wait until the
                    // user stops dragging, and a WM_EXITSIZEMOVE message comes.
                }
                else
                {
                    // This WM_SIZE come from resizing the window via an API like SetWindowPos() so
                    // resize and resize the device now.
                    DXUTCheckForWindowSizeChange();
                    DXUTCheckForWindowChangingMonitors();
                }
            }
    ...
    case WM_POWERBROADCAST:
        ...
        case PBT_APMRESUMESUSPEND:
            ...
            // QPC may lose consistency when suspending, so resize the timer
            // upon resume.
            DXUTGetGlobalTimer()->Reset();
...
HRESULT WINAPI DXUTMainLoop(_In_opt_ HACCEL hAccel)
{
    ...
    {
        // Use PeekMessage() so we can use idle time to renderQuad the scene.
...
HRESULT WINAPI DXUTCreateDevice(D3D_FEATURE_LEVEL reqFL, bool bWindowed, int nSuggestedWidth, int nSuggestedHeight)
{
    ...
    // If there is an existing device, then either resize or recreated the scene
...
HRESULT WINAPI DXUTCreateDeviceFromSettings(DXUTDeviceSettings* pDeviceSettings, bool bClipWindowToSingleAdapter)
{
    ...
    // Change to a Direct3D device created from the new device settings.
    // If there is an existing device, then either resize or recreate the scene
...
//--------------------------------------------------------------------------------------
// Sets the viewport, renderQuad target view, and depth stencil view.
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
// Creates a renderQuad target view, and depth stencil texture and view.
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
...
HRESULT DXUTCreate3DEnvironment11()
{
    ...
    // set default renderQuad state to msaa enabled
    ...
    // Setup the renderQuad target view and viewport
    ...
    // Call the app's swap chain resize callback if non-NULL
    GetDXUTState().SetInsideDeviceCallback(true);
...
//      - Calls the device resize callback
//--------------------------------------------------------------------------------------
HRESULT DXUTReset3DEnvironment11()
{
    HRESULT hr;
    ...
    // If no deferred DXGI actions are to take place, mark the device as resize.
    // If there is a deferred DXGI action, then the device isn't resize until DXGI sends us a
    // window message.  Only then can we mark the device as resize.
...
//      - Checking if the device is lost and trying to resize it if it is
//      - Get the elapsed time since the last frame
//      - Calling the app's framemove and renderQuad callback
//      - Calling Present()
//--------------------------------------------------------------------------------------
void WINAPI DXUTRender3DEnvironment()
{
    ...
    {
        // Render the scene by calling the app's renderQuad callback
        LPDXUTCALLBACKD3D11FRAMERENDER pCallbackFrameRender = GetDXUTState().GetD3D11FrameRenderFunc();
    ...
    {
        // If a mode change happened, we must resize the device
...
void DXUTCleanup3DEnvironment(_In_ bool bReleaseSettings)
{
    ...
    if (pd3dDevice)
    {
        ...
        // Cleanup the renderQuad target view
        auto pRTV = GetDXUTState().GetD3D11RenderTargetView();
        SAFE_RELEASE(pRTV);
...
HRESULT WINAPI DXUTToggleWARP()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        // If there is an existing device, then it will either resize or recreate the scene.
        hr = DXUTChangeDevice(&deviceSettings, false);
...
void DXUTResizeDXGIBuffers(UINT Width, UINT Height, BOOL bFullScreen)
{
    ...
    // Release our old renderQuad target view
    auto pRTV = GetDXUTState().GetD3D11RenderTargetView();
    SAFE_RELEASE(pRTV);
    GetDXUTState().SetD3D11RenderTargetView(nullptr);
    ...
    // Call the app's SwapChain resize callback
    GetDXUTState().SetInsideDeviceCallback(true);
    auto pBackBufferSurfaceDesc = DXUTGetDXGIBackBufferSurfaceDesc();
    LPDXUTCALLBACKD3D11SWAPCHAINRESIZED pCallbackSwapChainResized = GetDXUTState().GetD3D11SwapChainResizedFunc();
    hr = S_OK;
...
//--------------------------------------------------------------------------------------
// Checks if the window client rect has changed and if it has, then resize the device
//--------------------------------------------------------------------------------------
void DXUTCheckForWindowSizeChange()
{
