...
LRESULT CD3DArcBall::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        /*case WM_MOUSEHWHEEL:
        {
            ...
            mTranslationDelta = XMMatrixTranslation(0.0f, 0.0f, 5 * zDelta);
    ...
    case WM_MOUSEWHEEL:
    {
        ...
        XMMATRIX mTranslation = XMLoadFloat4x4(&m_mTranslation);
        mTranslation = XMMatrixMultiply(mTranslation, XMMatrixTranslation(0, 0, ((float)delta) / 220.0));
        XMStoreFloat4x4(&m_mTranslation, mTranslation);
    ...
    case WM_MOUSEMOVE:
    {
        ...
        else if ((MK_RBUTTON & wParam) || (MK_MBUTTON & wParam))
        {
            ...
            XMMATRIX mTranslationDelta = XMMatrixIdentity();
            XMMATRIX mTranslation = XMLoadFloat4x4(&m_mTranslation);
...
void CModelViewerCamera::FrameMove(_In_ float fElapsedTime)
{
    ...
    //if (m_bControlDownToZoom ? IsKeyDown(m_aKeys[CAM_CONTROLDOWN]) : true)
    ...
    // USER CODE
    XMMATRIX mTranslationPanArcBall = m_PanArcBall.GetTranslationMatrix();

    {
        ...
        DirectX::SimpleMath::Matrix(mTranslationPanArcBall).Decompose(scale, quat, trans);

        //TextOutDebug::drawString(
        //    std::to_string(trans.x) + "," +
        //    std::to_string(trans.y) + "," +
        //    std::to_string(trans.z)
        //);
    }

    // Update the view matrix _ USER CODE:'  * mTranslationPanArcBall'
    XMMATRIX mView = XMMatrixLookAtLH(vEye, vLookAt, vWorldUp) * mTranslationPanArcBall;
...
//  XMMATRIX mView = XMMatrixLookAtLH(vEye, vLookAt, vWorldUp);
    ...
    //// TODO: initLog code - user code
    //XMMATRIX mPan = m_PanArcBall.GetTranslationMatrix();

    ////mView *= mPan;
    ////vEye = XMVector3Transform(vEye, mPan);
    //// END: initLog code
...
void CModelViewerCamera::SetDragRect(_In_ const RECT& rc)
{
    ...
    m_PanArcBall.SetOffset(rc.left, rc.top);
...
LRESULT CModelViewerCamera::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        ((uMsg == WM_RBUTTONDOWN || uMsg == WM_RBUTTONDBLCLK) && m_nTranslateButtonMask & MOUSE_RIGHT_BUTTON))
    {
        int iMouseX = (short)LOWORD(lParam);
        int iMouseY = (short)HIWORD(lParam);
        m_PanArcBall.OnBegin(iMouseX, iMouseY);
    }
