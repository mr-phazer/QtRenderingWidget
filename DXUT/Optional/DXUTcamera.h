...
#include "..\..\RenderLib\TextOutDebug.h"
//--------------------------------------------------------------------------------------

class CD3DArcBall
{
    friend class CModelViewerCamera_Ext;
...
protected:
    ...
    INT m_nWidth;                           // arc ball's window
    ...
    DirectX::XMFLOAT4 m_qDown;              // DirectX::SimpleMath::Quaternion before button down
    DirectX::XMFLOAT4 m_qNow;               // Composite DirectX::SimpleMath::Quaternion for current drag
...
protected:
    ...
    DirectX::XMFLOAT3 m_vDefaultEye;        // Default kmera eye position
    ...
    bool m_bResetCursorAfterMove;           // If true, the class will resize the cursor position so that the cursor always has space to move
...
public:
    ...
    void SetButtonMasks(_In_ int nRotateModelButtonMask = MOUSE_MIDDLE_BUTTON, _In_ int nZoomButtonMask = MOUSE_WHEEL,
        ...
        _In_ int nTranskateButtonMask = MOUSE_MIDDLE_BUTTON
    )
    {
        ...
        m_nTranslateButtonMask = nTranskateButtonMask;
    }
    ...
    void SetKeyMasks(
        ...
        int kRotateCam = 0)
    {
        m_bControlDownToZoom = (bool)kZoom;
    }
    ...
    void SetWindow(_In_ int nWidth, _In_ int nHeight, _In_ float fArcballRadius = 0.9f)
    {
        ...
        m_PanArcBall.SetWindow(nWidth, nHeight, fArcballRadius);
    }
