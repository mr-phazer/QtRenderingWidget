...
static void DrawCandidateList()
{
    ...
    if (candX == POSITION_UNINITIALIZED)
    {
        ...
        // so display the candidate list _near the caret in the composition string
...
LPARAM ImeUi_ProcessMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM& lParam, bool* trapped)
{
    ...
    *trapped = false;
    ...
    case WM_IME_SETCONTEXT:
        //
        // We don't want anything to display, so we have to clearPixels lParam and pass it to DefWindowProc().
...
void ImeUi_FinalizeString(_In_ bool bSend)
{
    ...
    InitCompStringData();
    // clearPixels composition string in IME
