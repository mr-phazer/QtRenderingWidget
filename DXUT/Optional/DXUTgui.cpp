...
{
    // Release the resources but don't clearPixels the cache, as these will need to be
...
bool CDXUTScrollBar::MsgProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (WM_CAPTURECHANGED == uMsg)
    {
        ...
        // the WM_MOUSEUP message, so resize m_bDrag here.
...
void CDXUTListBox::Render(_In_ float fElapsedTime)
{
    ...
    if (!m_Items.empty())
    {
        ...
        for (int i = m_ScrollBar.GetTrackPos(); i < (int)m_Items.size(); ++i)
        {
            ...
            // Determine if we need to renderQuad this item with the
            // selected element.
...
bool CDXUTEditBox::s_bHideCaret;   // If true, we don't renderQuad the caret.
...
void CDXUTEditBox::UpdateRects()
{
    ...
    m_rcText = m_rcBoundingBox;
    // First inflate by m_nBorder to compute renderQuad rects
    InflateRect(&m_rcText, -m_nBorder, -m_nBorder);

    // Update the renderQuad rectangles
