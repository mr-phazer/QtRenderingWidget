...
public:
    ...
    void Clear() noexcept
    {
        ...
        memset(&m_xmVector, 0, sizeof(BANKDATA));
...
HRESULT WaveBankReader::Impl::Open(const wchar_t* szFileName) noexcept(false)
{
    ...
    wait = false;
    if (!ReadFile(hFile.get(), &m_xmVector, sizeof(m_xmVector), nullptr, &request))
    {
        DWORD error = GetLastError();
        if (error != ERROR_IO_PENDING)
            return HRESULT_FROM_WIN32(error);
    ...
    if (!result || (bytes != sizeof(m_xmVector)))
    {
        return HRESULT_FROM_WIN32(GetLastError());
    }

    if (be)
        m_xmVector.BigEndian();

    if (!m_xmVector.dwEntryCount)
    {
        return HRESULT_FROM_WIN32(ERROR_NO_DATA);
    }

    if (m_xmVector.dwFlags & BANKDATA::TYPE_STREAMING)
    {
        if (m_xmVector.dwAlignment < ALIGNMENT_DVD)
            return E_FAIL;
        if (m_xmVector.dwAlignment % DVD_SECTOR_SIZE)
            return E_FAIL;
    }
    else if (m_xmVector.dwAlignment < ALIGNMENT_MIN)
    {
        return E_FAIL;
    }

    if (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT)
    {
        if (m_xmVector.dwEntryMetaDataElementSize != sizeof(ENTRYCOMPACT))
        {
            return E_FAIL;
        }

        if (m_header.Segments[HEADER::SEGIDX_ENTRYWAVEDATA].dwLength > (MAX_COMPACT_DATA_SEGMENT_SIZE * m_xmVector.dwAlignment))
        {
    ...
    {
        if (m_xmVector.dwEntryMetaDataElementSize != sizeof(ENTRY))
        {
            return E_FAIL;
        }
    }
    ...
    if (metadataBytes != (m_xmVector.dwEntryCount * m_xmVector.dwEntryMetaDataElementSize))
    {
        return E_FAIL;
    }
    ...
    if (namesBytes > 0)
    {
        if (namesBytes >= (m_xmVector.dwEntryNameElementSize * m_xmVector.dwEntryCount))
        {
            ...
            for (uint32_t j = 0; j < m_xmVector.dwEntryCount; ++j)
            {
                DWORD n = m_xmVector.dwEntryNameElementSize * j;
    ...
    if (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT)
    {
        m_entries.reset(reinterpret_cast<uint8_t*>(new (std::nothrow) ENTRYCOMPACT[m_xmVector.dwEntryCount]));
    }
    else
    {
        m_entries.reset(reinterpret_cast<uint8_t*>(new (std::nothrow) ENTRY[m_xmVector.dwEntryCount]));
    }
    if (!m_entries)
        return E_OUTOFMEMORY;
    ...
    if (be)
    {
        if (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT)
        {
            ...
            for (size_t j = 0; j < m_xmVector.dwEntryCount; ++j, ++ptr)
                ptr->BigEndian();
        }
        else
        {
    ...
    else
    {
        ...
        if (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT)
        {
            if (m_xmVector.CompactFormat.wFormatTag == MINIWAVEFORMAT::TAG_XMA)
                xma = true;
        }
        else
        {
...
HRESULT WaveBankReader::Impl::GetFormat(uint32_t index, WAVEFORMATEX* pFormat, size_t maxsize) const noexcept
{
    ...
    if (index >= m_xmVector.dwEntryCount || !m_entries)
    {
        return E_FAIL;
    }

    auto& miniFmt = (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT) ? m_xmVector.CompactFormat : (reinterpret_cast<const ENTRY*>(m_entries.get())[index].Format);
    ...
    switch (miniFmt.wFormatTag)
    {
        ...
        case MINIWAVEFORMAT::TAG_XMA: // XMA2 is supported by Xbox One
            ...
            pFormat->cbSize = sizeof(XMA2WAVEFORMATEX) - sizeof(WAVEFORMATEX);
            {
                ...
                auto seekTable = FindSeekTable(index, m_seekData.get(), m_header, m_xmVector);
                if (seekTable)
                {
                ...
                if (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT)
                {
                    ...
                    entry.ComputeLocations(dwOffset, dwLength, index, m_header, m_xmVector, reinterpret_cast<const ENTRYCOMPACT*>(m_entries.get()));

                    xmaFmt->SamplesEncoded = entry.GetDuration(dwLength, m_xmVector, seekTable);
...
HRESULT WaveBankReader::Impl::GetMetadata(uint32_t index, Metadata& metadata) const noexcept
{
    ...
    if (m_xmVector.dwFlags & BANKDATA::FLAGS_COMPACT)
    {
        ...
        auto seekTable = FindSeekTable(index, m_seekData.get(), m_header, m_xmVector);
        metadata.duration = entry.GetDuration(dwLength, m_xmVector, seekTable);
...
{
    return (pImpl->m_xmVector.dwFlags  & BANKDATA::TYPE_STREAMING) != 0;
}
...
{
    return pImpl->m_xmVector.szBankName;
}


uint32_t WaveBankReader::Count() const noexcept
{
    return pImpl->m_xmVector.dwEntryCount;
}
...
{
    return (pImpl->m_xmVector.dwFlags & BANKDATA::TYPE_STREAMING) ? pImpl->m_async : INVALID_HANDLE_VALUE;
}
