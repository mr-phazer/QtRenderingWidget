...
#else
	void CreateDynamicBuffer(_In_ ID3D11Device* device, uint32_t bufferSize, D3D11_BIND_FLAG bindFlag, _Outptr_ ID3D11Buffer** m_vecRawBuffer)
	{
		D3D11_BUFFER_DESC desc = {};
		...
		ThrowIfFailed(
			device->CreateBuffer(&desc, nullptr, m_vecRawBuffer)
		);

		assert(m_vecRawBuffer != nullptr && *m_vecRawBuffer != nullptr);
		_Analysis_assume_(m_vecRawBuffer != nullptr && *m_vecRawBuffer != nullptr);

		SetDebugObjectName(*m_vecRawBuffer, "DirectXTK:PrimitiveBatch");
	}
#endif
}
