...
namespace DirectX
{
	...
	HRESULT __cdecl CreateStaticBuffer(_In_ ID3D11Device* device,
		...
		_COM_Outptr_ ID3D11Buffer** m_vecRawBuffer) noexcept;
	...
	HRESULT CreateStaticBuffer(_In_ ID3D11Device* device,
		...
		_COM_Outptr_ ID3D11Buffer** m_vecRawBuffer) noexcept
	{
		return CreateStaticBuffer(device, data, count, sizeof(T), bindFlags, m_vecRawBuffer);
	}
	...
	HRESULT CreateStaticBuffer(_In_ ID3D11Device* device,
		...
		_COM_Outptr_ ID3D11Buffer** m_vecRawBuffer) noexcept
	{
		return CreateStaticBuffer(device, data.data(), data.size(), sizeof(typename T::value_type), bindFlags, m_vecRawBuffer);
	}
	...
	namespace Internal
	{
		...
		{
		protected:
			void __cdecl CreateBuffer(_In_ ID3D11Device* device, size_t bytes, _Outptr_ ID3D11Buffer** m_vecRawBuffer);
		};
	}
