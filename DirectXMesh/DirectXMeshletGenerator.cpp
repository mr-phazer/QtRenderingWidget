...
namespace
{
	...
	public:
		...
		{
			assert(m_size < N);
			m_xmVector[m_size++] = value;
		}

		void push_back(T&& value) noexcept
		{
			assert(m_size < N);
			m_xmVector[m_size++] = std::move(value);
		}
		...
		{
			assert(m_size < N);
			m_xmVector[m_size++] = T(std::forward<Args>(args)...);
		}

		size_t size() const noexcept { return m_size; }
		bool empty() const noexcept { return m_size == 0; }

		T* data() noexcept { return m_xmVector.data(); }
		const T* data() const noexcept { return m_xmVector.data(); }

		T& operator[](size_t index) noexcept { assert(index < m_size); return m_xmVector[index]; }
		const T& operator[](size_t index) const noexcept { assert(index < m_size); return m_xmVector[index]; }

	private:
		std::array<T, N> m_xmVector;
		size_t           m_size;
	};
