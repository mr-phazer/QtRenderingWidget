...
namespace rldx
{
	class DxCameraOrbital
	{
		...
		float m_fCameraLookAtMoveSpeed = 0.01f;
	...
	public:
		...
		sm::Vector3 WorldSpaceToScreenSpace(const sm::Vector3& worldPosition, const sm::Matrix& worldMatrix)
		{
			...
			sm::Viewport viewPort(0.f, 0.f, static_cast<float>(m_width), static_cast<float>(m_height));
		...
		struct GeometryData
		{
			...
			float m_world_yaw = 0.f; // DirectX::XM_PIDIV4;;
			float m_world_pitch = 0.f; // -DirectX::XM_PIDIV4;
			float m_world_roll = 0.f;
			float m_zoom = 0.f;
			...
			float fRadius = 0.1f;                        // Distance from the camera to model
			float fDefaultRadius = 1.0f;                 // Distance from the camera to model
